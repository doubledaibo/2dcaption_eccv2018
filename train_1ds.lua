...
cmd:option('-learning_rate_decay_every', 50, '')
...
if opt.start_from ~= '' then -- just copy to gpu1 params
	...
	protos.cnn_fc = net_utils.build_residual_cnn_fc(cnn_raw, {backend = cnn_backend}):cuda()
end
...
local function evaluate_split(split)
	...
	for i = 1, nbatch do
		...
		local logprobs = protos.lm:forward({expanded_feats_fc, data.labels})
		local loss = protos.crit:forward({logprobs, data.labels})

		loss_sum = loss_sum + loss 
		n = n + protos.crit.n
		-- forward the model to also get generated samples for each image
		local seq = protos.lm:sample_beam({feats_fc}, {beam_size = 3})
...
local function Train(epoch)
	...
	for i = 1, opt.iter_size do
		...
		local log_prob = protos.lm:forward({expanded_feats_fc, data.labels})
		loss = loss + protos.crit:forward({log_prob, data.labels})
		n = n + protos.crit.n
		local d_logprobs = protos.crit:backward({})
		local dexpanded_fc = protos.lm:backward({expanded_feats_fc, data.labels}, d_logprobs)
		...
		if epoch >= opt.finetune_cnn_after and opt.finetune_cnn_after ~= -1 then
			...
			dconv = protos.cnn_fc:backward(feats_conv, dfc)
