...
for n = 1, nbatch do
	...
	local logprobs, seqs, seqs_deact, sents, sents_deact
	seqs = protos.lm:sample_beam({feats_conv}, opt)
	sents = net_utils.decode_sequence(vocab, seqs)
	seqs_deact = protos.lm:sample_beam_deactivate({feats_conv}, opt) 
	sents_deact = net_utils.decode_sequence(vocab, seqs_deact)
	...
	for k = 1, #sents do	
		...
		if imgId_cell[img_id] == nil then
			imgId_cell[img_id] = 1
			entry = {image_id = img_id, caption = sents[k], caption_deact = sents_deact[k]}
